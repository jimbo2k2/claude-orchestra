...
## Tech Stack
- Runtime: Bash (no external dependencies)
- Testing: `bash tests/run.sh`
- No framework, no package manager

## Architecture
```
scripts/        — Bash scripts (one per TODO task)
tests/
└── run.sh      — Test runner with assertions for all scripts
```

## Conventions
- All scripts use `set -euo pipefail`
- Scripts must be executable (`chmod +x`)
- Keep scripts simple and focused — one purpose per file
- Use descriptive variable names

## Testing Requirements
- Run the test suite after completing each logical change: `bash tests/run.sh`
- Do NOT proceed to the next change if tests are failing
- When adding new functionality, ensure corresponding tests pass
- Test command: `bash tests/run.sh`
...
- Do NOT modify `.claude/settings.json` or files in the `scripts/` directory under `~/claude-scripts/`
